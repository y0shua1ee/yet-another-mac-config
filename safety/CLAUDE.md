...
- `synthetic-sentinel-passed` 只是内层 fixture 结果；唯一允许的 scoped claim 是 `covered-surfaces-unchanged-for-run`，且只来自完整 outer real evidence。禁止整机、当前 host readiness、多机或 fresh-install claim。
