...
task、wave、phase 的 hard deadline 分别是 15、47、305 秒；完整公式是 `6 * 47 + 15 + 8 = 305`。每次 public `test.sh` 调用都必须无条件建立一个 watchdog，并让 setup、固定 docs checks、build/list/test、child dispatch 与 marker-owned cleanup 全部计入预算。受监控 body 只能由 watchdog 从同一文件的固定 marker 后读取，大小有上限，并由最外层 watchdog 为本次 public invocation 建立唯一 PGID supervisor；禁止重新引入任何 caller-selectable internal/re-exec 参数，或使用 ambient environment、PID、继承 FD、nonce 来关闭 watchdog。wave/phase 只能通过同一 embedded body 内的 fixed internal task/wave dispatcher 聚合；每个 child 必须创建 fresh root/cache，但不得递归 public `test.sh`、不得调用 `setpgrp`/`setsid` 或建立 child supervisor。唯一 supervisor 必须通过 fork 前建立的匿名私有 socket 和同步 fixed-kind 协议管理 monotonic deadline stack：embedded task 固定 15 秒、phase component wave 固定 47 秒，子层只能缩短父层 deadline，剩余预算不足时不得启动 child。socket 协议消息不得携带 duration，production public argv/env 也不得接受 FD、token 或 bypass；test-only override 只能在显式测试模式下由 supervisor 预校验，并将对应 production ceiling 缩短到 500 毫秒或以上，绝不能提高或关闭。聚合层只能原样传播唯一 timeout envelope，不得再叠加命令级 process-group wrapper。最外层 timeout authority 必须终止同一 PGID 中的 nested body/helper，清理所有本次 marker-owned root，只输出一个 `runner-deadline-exceeded` envelope 并原样返回 `124`。`docs-and-phase-gate` 只能做固定文档/symlink/结构检查，不得调用 phase；`phase-integration` 只能依次启动 `phase-e2e` 与 `docs-and-phase-gate`，不得重复 phase。完整 phase 必须单独运行，并保持六个固定组件 wave 后接 `phase-e2e` 的顺序。
...
- 每次 workflow 必须在第一个 repository input 前只捕获一次 HEAD commit OID 与大小受限的完整 stage-0 index snapshot；所有输入共享该 frozen view，结束前再次核对 HEAD、完整 index bytes 与 root identity。worktree 读取必须从 retained root handle 开始逐组件执行 `Lstat` → `OpenRoot` → opened/named `SameFile` recheck 并保留目录 handle，最终文件使用 no-follow、nonblocking、before/opened/after identity recheck。固定 `/usr/bin/git` 的无网络、无 hook exact plumbing 必须证明唯一 stage-0 entry、index 与 frozen HEAD mode/blob 一致、只有 owner execute `0100` 把 worktree mode 映射为 `100755`、HEAD blob 与实际消费的 bounded bytes 一致。Git 缺失/失败、非 worktree、untracked、ignored、任一 symlink、目录/文件替换、跨输入 HEAD/index 混用、index substitution、bytes substitution 或 chmod-only mode drift 必须在创建 fixture/store 前 fail closed。
