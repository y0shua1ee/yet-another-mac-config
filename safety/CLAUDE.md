...
task、wave、phase 的 hard deadline 分别是 15、47、305 秒；完整公式是 `6 * 47 + 15 + 8 = 305`。每个 runner 必须从脚本入口开始只拥有一个 watchdog，并让 setup、固定 docs checks、build/list/test、child dispatch 与 marker-owned cleanup 全部计入预算；内部 re-exec 只接受 direct-parent + random nonce + private inherited pipe FD 的完整握手，任何 caller ambient/stale guard 都必须重建 watchdog。父聚合层只能原样传播自限时 child 的唯一 timeout envelope，不得再叠加命令级 process-group wrapper。timeout 必须只输出一个 `runner-deadline-exceeded` envelope 并原样返回 `124`。`docs-and-phase-gate` 只能做固定文档/symlink/结构检查，不得调用 phase；`phase-integration` 只能依次启动 `phase-e2e` 与 `docs-and-phase-gate`，不得重复 phase。完整 phase 必须单独运行，并保持六个固定组件 wave 后接 `phase-e2e` 的顺序。
...
- 所有 repository input 必须通过固定 `/usr/bin/git` 的无网络、无 hook exact plumbing：无 symlink regular file、唯一 stage-0 index entry、index 与 frozen HEAD mode/blob 一致、HEAD blob 与实际消费的 bounded worktree bytes 一致。Git 缺失/失败、非 worktree、untracked、ignored、symlink、index substitution 或 worktree substitution 必须在创建 fixture/store 前 fail closed。
