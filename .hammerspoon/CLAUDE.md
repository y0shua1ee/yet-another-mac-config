...
- **Hammerspoon app itself** — installed via the `hammerspoon` cask declared in `nix/darwin/homebrew.nix`, so `./sync_mac.sh` installs it. On machines that don't use Nix, install manually with `brew install --cask hammerspoon`.
...
- Home Manager (`nix/home/dotfiles.nix`) only links `.hammerspoon` into `~/.hammerspoon`; the Accessibility permission above is granted manually and is in addition to running the sync script.
...
- `init.lua` and any Spoons stay in `.hammerspoon/` as plain files linked by Home Manager; do not rewrite them into a Home Manager module or move them under `nix/`.
