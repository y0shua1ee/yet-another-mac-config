...
> 历史说明：本文件记录 Phase 3 当时的渐进式迁移计划。2026-07-14 起，Determinate Nix + nix-darwin + Home Manager 已成为唯一主控制面；旧版 `setup_mac.sh` 已退役，oh-my-tmux 主配置也已改由 flake 锁定。当前操作说明以 [`README.md`](./README.md) 为准。
...
3. 当时保留手工链接回退；该过渡边界现已由 Home Manager 主控制面取代。
...
- tmux 在 Phase 3B 当时的事实源边界如下，现已由 `nix/home/tmux.nix` 的叶子文件接管取代：
  - `~/.config/tmux/tmux.conf.local` 继续是用户自定义入口，跟踪在 git 中。
  - `~/.config/tmux/tmux.conf` 现由 Home Manager 指向 `flake.lock` 固定的 oh-my-tmux source。
