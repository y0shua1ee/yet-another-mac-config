...
- Phase 5A / 5B 的语言栈边界：
  ...
  - `.config/mise/config.toml` 是仓库跟踪的全局 fallback 配置；当前机器已执行 `mise trust -C ~/Documents/dev/config/yet-another-mac-config`。新机器或路径迁移后如出现 untrusted 提示，重新 trust 当前仓库路径。
  - Phase 5D 完成：NVM 旧版本 + Homebrew `nvm` + `~/.nvm` 已全部移除，`~/.zshrc.local` 中 NVM 加载已注释。Node / Go 完全由 mise 管理。
